...
- Cover headers, bodies, trailers, mode overrides,
  ...
  sequences, route-cache clearing, and fail-open /
  fail-closed.
- Cover supported MaaS profiles, models, providers,
  translations, credentials, state updates, and
...
- Verify overload rejection is deliberate and
  bounded, and that recovery occurs without
  restart.
- Emit machine-readable qualification results with
  exact Envoy, IPP baseline, Praxis, configuration,
  and fixture revisions recorded.
...
stays bounded at target concurrency.
...
- As a security auditor, I want security-negative
  cases (forged headers, unauthorized requests)
  verified so that policy bypass cannot ship
  undetected.
