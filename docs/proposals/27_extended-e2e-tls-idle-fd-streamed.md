...
`deploy/overlays/e2e-extended/`; each run records `tls_validation_mode` and
`tls_negative_scenario` in metadata.
...
| Scenario class | Setup | Pass criteria |
| --- | --- | --- |
| Wrong / expired **server** cert (Envoy → ext-proc TLS) | Negative overlay under `deploy/overlays/e2e-extended/tls/` | Client receives **HTTP 503**; access log (via `access-log-tls-negative.yaml`) has `%RESPONSE_CODE%` **503** and `%RESPONSE_CODE_DETAILS%` matching **`ext_proc`** / **`ext_proc_error`** (Envoy local reply from the filter); response **must not** include `x-qualification-request-sha256-hop-*` |
| Missing / invalid **client** cert (mTLS enabled) | Negative client cert or omit client cert in overlay | Same **503 + ext_proc error details** contract; hop oracle headers **absent** |
| Valid baseline (control) | Matching positive overlay in the same matrix row | **HTTP 200** (or profile-expected success) **with** hop oracle headers when body-oracle is enabled |
...
upstream failure). Each negative run publishes `tls_negative_scenario` (for
example `server_wrong_san`, `server_expired`, `client_missing`) in
`report.json` for audit.
