...
//! it only comes up on a FIPS host. The listener probes of the FIPS suite
//! (`tests/fips/main.rs`, the `listener_` tests, including a real ExtProc
//! gRPC exchange) are then pointed at it through `PRAXIS_FIPS_PROBE_ADDR`,
//! and run inside the toolchain image (`make fips-toolchain`) so the host
//! needs podman and nothing else; `--host-cargo` runs them with this host's
//! cargo instead.
...
/// The gRPC listener port inside the container.
const CONTAINER_PORT: u16 = 50_051;
...
/// The tests to run: the listener probes of the FIPS suite.
const TEST_FILTER: &str = "listener_";

/// The feature set the FIPS build ships (the Makefile's `FIPS_FEATURES`), so
/// the probes compile the suite exactly as `make test-fips` does and share
/// its build cache.
const FIPS_FEATURES: &str = "responses,aws-sigv4";
...
pub(crate) struct Args {
    ...
    #[arg(long, default_value = "praxis-extproc-fips-toolchain", value_name = "IMAGE")]
...
impl Container {
    ...
    fn start(image: &str, work: &Path, port: u16) -> Result<Self, String> {
        let name = format!("praxis-extproc-fips-probe-{}", std::process::id());
    ...
    fn wait_until_serving(&self, port: u16) -> Result<(), String> {
        ...
        loop {
            ...
            if let Some(log) = self.refusal()? {
                return Err(format!(
                    "the image refuses to serve; under PRAXIS_REQUIRE_FIPS=1 that means the host is not in FIPS \
                     mode. Its log:\n{log}"
                ));
            }
    ...
    /// The log, when it carries the `PRAXIS_REQUIRE_FIPS` refusal: the
    /// server does not exit on it, it stays up and inspectable (health
    /// `NotServing`) without serving gRPC, so the log is what says the
    /// requirement was not met.
    fn refusal(&self) -> Result<Option<String>, String> {
        let log = self.logs()?;
        Ok(log.contains("refusing to serve").then_some(log))
    }
...
fn host_cargo_test(addr: &str, work: &Path) -> std::io::Result<std::process::ExitStatus> {
    Command::new(env!("CARGO"))
        ...
        .args(["-p", "praxis-extproc", "--test", "fips", "--", TEST_FILTER])
...
/// The ExtProc config, at the paths the container sees: the gRPC listener
/// on TLS with the probe certificate, sidecars on loopback, a headers-only
/// pipeline for the probes' real ExtProc exchange.
fn write_config(work: &Path) -> Result<(), String> {
    let config = format!(
        "server:\n  grpc_address: \"0.0.0.0:{CONTAINER_PORT}\"\n  health_address: \"127.0.0.1:50052\"\n  \
...
mod tests {
    ...
    fn the_status_line_must_report_fips_on_both_signals() {
        let good = "\u{1b}[2m2026-09-24T00:00:00Z\u{1b}[0m INFO praxis_extproc::fips: installed rustls crypto provider \
