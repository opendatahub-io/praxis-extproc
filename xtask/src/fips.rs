...
//! - `host-check`: attest that a host is in FIPS mode as the module's Security Policy requires, and that the FIPS image
//!   carries a validated module build.
//! - `runtime-probe`: run the shipped image on a FIPS host and drive the listener probes against it.
//!
...
//! validated module builds) is compiled in from `xtask/assets/fips/`, so
//! nothing depends on host files.
