...
use crate::{
    adapter,
    maas::{BbrProcessor, RoutingError},
    metrics,
...
async fn run_request_pipeline(
    ...
    if let Some(processor) = ctx.extensions.get::<BbrProcessor>() {
        let applied = match apply_bbr(
            ...
            Ok(applied) => applied,
            Err(error) => return Ok(vec![response::immediate(bbr_routing_rejection(&error))]),
        };
...
/// # Errors
///
/// Returns [`RoutingError`] when the model is empty, unknown, or the provider is unusable.
fn apply_bbr(
    processor: &BbrProcessor,
    model: Option<&str>,
    path: &str,
    headers: &HeaderMap,
) -> Result<AppliedBbr, RoutingError> {
...
/// Build an HTTP 400 [`ImmediateResponse`] for a BBR routing failure.
fn bbr_routing_rejection(error: &RoutingError) -> ImmediateResponse {
    metrics::record_immediate_response();
    adapter::rejection_to_immediate(&praxis_filter::Rejection::status(400).with_body(Bytes::from(error.to_string())))
}
