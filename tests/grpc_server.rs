...
#[tokio::test]
async fn bbr_unknown_model_returns_immediate_response() {
    let (mut client, _shutdown) = start_server(BBR_CONFIG).await;

    let body = br#"{"model":"unknown-model"}"#;
    let responses = send_full_request(&mut client, "POST", "/chat/completions", body).await;

    let immediate = responses.iter().find_map(|r| match &r.response {
        Some(RespVariant::ImmediateResponse(imm)) => Some(imm),
        _ => None,
    });
    assert!(
        immediate.is_some(),
        "unknown model should return ImmediateResponse, got {responses:?}"
    );
    let immediate = immediate.unwrap();
    assert_eq!(
        immediate.status.as_ref().map(|s| s.code),
        Some(400),
        "unknown model should be HTTP 400, got {immediate:?}"
    );
    assert!(
        immediate.body.contains("model not found"),
        "body should explain the routing error, got {:?}",
        immediate.body
    );
}
