#!/usr/bin/env bash
# Validate MaaS + praxis-extproc: deployment health, auth enforcement, model inference.
set -euo pipefail
...
echo -e "${BOLD}MaaS + praxis-extproc lab validation${NC}"
...
check "payload-processing (praxis-extproc) ready" \
