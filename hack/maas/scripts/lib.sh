#!/usr/bin/env bash
# Shared helpers for hack/maas scripts.
set -euo pipefail
...
MAAS_CONTROLLER_IMAGE="${MAAS_CONTROLLER_IMAGE:-localhost/maas-controller:dev}"
...
parse_context() {
  if [[ $# -lt 1 || -z "${1:-}" ]]; then
    die "usage: $0 <kube-context>  (e.g. kind-maas-praxis-local)"
  fi
  KUBE_CONTEXT="$1"
  if [[ "$KUBE_CONTEXT" == kind-* ]]; then
    KIND_CLUSTER_NAME="${KUBE_CONTEXT#kind-}"
  else
    KIND_CLUSTER_NAME="${KIND_CLUSTER_NAME:-maas-praxis-local}"
  fi
